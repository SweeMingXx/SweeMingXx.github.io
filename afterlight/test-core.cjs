...
let a=new Game(42,'zen'),b=new Game(42,'zen');for(let i=0;i<3600;i++){const input={x:Math.sin(i/120),y:Math.cos(i/120)};a.step(input);b.step(input)}assert.deepEqual(a,b);assert.equal(a.time,60);assert.equal(a.ended,true);assert.equal(a.echoes.length,6);assert.equal(a.shield,3);
